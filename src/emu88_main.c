...
#include "../i88/iosys.inc"
};

const unsigned char rom1[] = {
#include "../i88/msdos.inc"
};
...
//	div_t ans;
    ...
    printf("Use PWM3  %.2fMHz(Duty %.1f%%) for 8088 clock\n\r", 
    			1/((PWM3PR+1)*P64)*1000, (double)(PWM3S1P1*100/(PWM3S1P1+PWM3S1P2)));
    printf("\n\r");
	...
	/* IO.SYS */
	write_sram(IOSYS_OFF, (uint8_t *)rom0, sizeof(rom0));
	printf("Load IO.SYS = %06lx, File size = %04x\r\n", (uint32_t)IOSYS_OFF, sizeof(rom0));

	/* MSDOS.SYS */
//	ans = div(sizeof(rom0)+SECTOR_SIZE, SECTOR_SIZE);
//	msdos_off = (uint16_t)(ans.quot+IOSYS_OFF);
	msdos_off = (((sizeof(rom0)+SECTOR_SIZE) / SECTOR_SIZE)+1)*SECTOR_SIZE+IOSYS_OFF;
